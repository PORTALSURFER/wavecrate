...
use crate::native_app::app_chrome::center_panel::{
    library_sidebar_overlays, library_sidebar_region, library_sidebar_resize_overlay,
    metadata_tag_library_region, metadata_tag_library_region_visible, sample_workspace_overlays,
    sample_workspace_region,
};
...
pub(in crate::native_app) fn shell(state: &NativeAppState) -> ui::View<GuiMessage> {
    let mut shell = ui::workspace_shell(sample_workspace_region(state))
        ...
        .workspace_overlay(library_sidebar_resize_overlay(state))
