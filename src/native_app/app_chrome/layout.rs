...
use crate::native_app::app_chrome::center_panel::{
    active_workspace_overlays, library_sidebar_region, metadata_tag_library_region,
    metadata_tag_library_region_visible, sample_workspace_region,
};
...
pub(in crate::native_app) fn shell(state: &NativeAppState) -> ui::View<GuiMessage> {
    let mut shell = ui::workspace_shell(sample_workspace_region(state))
        ...
        .leading_sidebar(library_sidebar_region(state))
        .status_bar(bottom_status_area(state))
        .overlays(active_workspace_overlays(state))
        .outer_spacing(0.0);
