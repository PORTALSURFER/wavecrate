...
pub(super) const STARMAP_VIEW_TOOLTIP: &str = "Switch between list and Starmap views.";
...
impl<'a> SampleBrowserHeaderProjection<'a> {
    pub(super) fn from_model(model: SampleBrowserHeaderBar<'a>) -> Self {
        Self {
            ...
            map_view: StarmapViewButtonProjection::new(model.map_view_active),
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(super) struct StarmapViewButtonProjection {
    pub(super) active: bool,
    pub(super) tooltip: &'static str,
}

impl StarmapViewButtonProjection {
    pub(super) fn new(active: bool) -> Self {
        Self {
            active,
            tooltip: STARMAP_VIEW_TOOLTIP,
        }
    }
}
