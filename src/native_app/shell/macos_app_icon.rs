...
#[cfg(target_os = "macos")]
pub(in crate::native_app) fn install_wavecrate_application_icon() {
    if let Err(error) = install_wavecrate_application_icon_from_path(&wavecrate_app_icon_path()) {
...
#[cfg(target_os = "macos")]
fn install_wavecrate_application_icon_from_path(path: &Path) -> Result<(), String> {
...
mod tests {
    ...
    fn bundled_macos_icon_path_exists() {
        assert!(wavecrate::sample_sources::harvest_file_ops::path_is_file(
            &wavecrate_app_icon_path()
        ));
    }
}
