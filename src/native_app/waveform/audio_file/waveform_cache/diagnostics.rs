...
const SLOW_CACHE_PHASE_THRESHOLD: Duration = Duration::from_millis(8);
...
mod tests {
    ...
    #[test]
    fn slow_waveform_cache_phase_log_claim_is_rate_limited() {
        LAST_SLOW_CACHE_PHASE_LOG_MS.store(5_000, Ordering::Relaxed);

        assert!(!claim_slow_cache_phase_log(5_100));
        assert!(claim_slow_cache_phase_log(5_250));
        assert!(!claim_slow_cache_phase_log(5_300));
    }
}
