...
fn top_status_bar_replaces_text_labels_with_volume_slider_and_audio_pill() {
    ...
    state.audio_output_resolved = Some(crate::native_app::ResolvedOutput {
    ...
    let frame = crate::native_app::top_status_bar(&state)
    ...
    let slider_fills = frame
        .paint_plan
        .visible_fill_rects_for_widget(crate::native_app::VOLUME_SLIDER_ID)
        .count();
...
fn volume_slider_drag_emits_normalized_volume() {
    assert_eq!(
        crate::native_app::audio_settings::volume_slider(0.25).view_dispatch_widget_output(
            crate::native_app::VOLUME_SLIDER_ID,
            radiant::widgets::WidgetOutput::typed(radiant::widgets::SliderMessage::ValueChanged {
                value: 0.75
            },),
        ),
        Some(crate::native_app::GuiMessage::SetVolume(0.75))
    );
}
...
fn default_gui_volume_drag_defers_config_persistence_until_debounce() {
    ...
    assert!((loaded.core.volume - crate::native_app::DEFAULT_VOLUME).abs() < f32::EPSILON);
...
fn audio_engine_pill_activates_settings_toggle() {
    ...
    let surface = crate::native_app::top_status_bar(&state).into_surface();
    let pill = surface
        .find_widget(crate::native_app::AUDIO_ENGINE_PILL_ID)
        .and_then(|widget| {
            widget
                .widget_object()
                .as_any()
    ...
    assert_eq!(
        surface.dispatch_widget_output(
            crate::native_app::AUDIO_ENGINE_PILL_ID,
            radiant::widgets::WidgetOutput::typed(BadgeMessage::Activate),
        ),
        Some(crate::native_app::GuiMessage::ToggleAudioSettings)
    );
}
...
fn general_settings_button_opens_general_tab() {
    let mut state = NativeAppState::load_default().expect("default state loads");
    state.audio_settings_open = true;
    state.app_settings_tab = crate::native_app::AppSettingsTab::General;
    let surface = crate::native_app::top_status_bar(&state).into_surface();
    let button = surface
        .find_widget(crate::native_app::GENERAL_SETTINGS_BUTTON_ID)
        .and_then(|widget| {
            widget
                .widget_object()
                .as_any()
    ...
    assert_eq!(
        surface.dispatch_widget_output(
            crate::native_app::GENERAL_SETTINGS_BUTTON_ID,
            radiant::widgets::WidgetOutput::typed(ButtonMessage::Activate),
        ),
        Some(crate::native_app::GuiMessage::OpenGeneralSettings)
    );
}
...
fn settings_top_bar_actions_open_expected_tabs() {
    ...
    state.apply_message(
        crate::native_app::GuiMessage::OpenGeneralSettings,
        &mut context,
    );
    assert!(state.audio_settings_open);
    assert_eq!(
        state.app_settings_tab,
        crate::native_app::AppSettingsTab::General
    );

    state.apply_message(
        crate::native_app::GuiMessage::ToggleAudioSettings,
        &mut context,
    );
    assert!(state.audio_settings_open);
    assert_eq!(
        state.app_settings_tab,
        crate::native_app::AppSettingsTab::AudioEngine
    );
...
fn audio_settings_snapshot_uses_cached_device_options() {
    let mut state = gui_state_for_span_tests();
    state.audio_hosts = vec![crate::native_app::AudioHostSummary {
...
fn audio_sample_rate_label_matches_status_chip_format() {
    assert_eq!(
        crate::native_app::format_sample_rate_label(48_000),
        "48 kHz"
    );
    assert_eq!(
        crate::native_app::format_sample_rate_label(44_100),
        "44.1 kHz"
    );
    assert_eq!(crate::native_app::format_sample_rate_label(960), "960 Hz");
}
...
fn settings_window_shows_audio_engine_tab_controls() {
    ...
    state.app_settings_tab = crate::native_app::AppSettingsTab::AudioEngine;
    state.audio_hosts = vec![crate::native_app::AudioHostSummary {
        id: String::from("asio"),
        label: String::from("ASIO"),
        is_default: false,
    ...
    let frame = crate::native_app::audio_settings_popover(&state)
