...
impl NativeAppState {
    pub(in crate::native_app) fn clear_loaded_sample_if_exact(&mut self, path: &Path) {
        if self.waveform.current.path() == path {
            self.clear_loaded_sample_after_trash();
        }
    }

    pub(in crate::native_app) fn clear_loaded_sample_if_path_within(&mut self, root: &Path) {
