//! Top-level native app chrome: layout, toolbar, library views, status bar, and waveform shell.

pub(in crate::native_app) mod browser_context_menu;
pub(in crate::native_app) mod layers;
pub(in crate::native_app) mod layout;
pub(in crate::native_app) mod library_browser;
pub(in crate::native_app) mod metadata_tag_library;
pub(in crate::native_app) mod modals;
pub(in crate::native_app) mod overlays;
pub(in crate::native_app) mod presentation;
pub(in crate::native_app) mod settings;
pub(in crate::native_app) mod status_bar;
pub(in crate::native_app) mod toolbar;
pub(in crate::native_app) mod view_models;
pub(in crate::native_app) mod waveform_panel;
