...
impl GuiAppState {
    pub(in crate::gui_app) fn normalize_selected_samples(
        &mut self,
    ...
    pub(in crate::gui_app) fn apply_normalization_progress(
        &mut self,
    ...
    pub(in crate::gui_app) fn finish_normalization(&mut self, result: NormalizationResult) {
        let started_at = Instant::now();
        if self
            .normalization_progress
            .as_ref()
