...
pub(in crate::gui_app) type TransactionResult = Result<(), String>;

pub(in crate::gui_app) struct TransactionAction<T> {
    label: String,
...
pub(in crate::gui_app) struct Transaction<T> {
    id: u64,
    label: String,
...
pub(in crate::gui_app) enum TransactionListState {
    Active,
    Undoable,
    Redoable,
}
...
impl TransactionListState {
    pub(in crate::gui_app) fn label(self) -> &'static str {
        match self {
            Self::Active => "Open",
            Self::Undoable => "Undo",
            Self::Redoable => "Redo",
...
pub(in crate::gui_app) struct TransactionListItem {
    pub(in crate::gui_app) id: u64,
    pub(in crate::gui_app) label: String,
    pub(in crate::gui_app) action_count: usize,
    pub(in crate::gui_app) action_labels: Vec<String>,
    pub(in crate::gui_app) state: TransactionListState,
}

#[derive(Clone, Debug, PartialEq, Eq)]
pub(in crate::gui_app) struct TransactionApplied {
    pub(in crate::gui_app) label: String,
    pub(in crate::gui_app) action_count: usize,
}

pub(in crate::gui_app) struct TransactionHistory<T> {
...
impl<T> TransactionHistory<T> {
    pub(in crate::gui_app) fn new() -> Self {
        Self {
            undo: VecDeque::new(),
            redo: VecDeque::new(),
            active: None,
    ...
    pub(in crate::gui_app) fn begin_transaction(&mut self, label: impl Into<String>) {
    ...
    pub(in crate::gui_app) fn commit_transaction(&mut self) -> bool {
    ...
    pub(in crate::gui_app) fn register_action(
        &mut self,
        label: impl Into<String>,
        undo: impl Fn(&mut T) -> TransactionResult + 'static,
        redo: impl Fn(&mut T) -> TransactionResult + 'static,
    ...
    pub(in crate::gui_app) fn push_transaction(
        &mut self,
        label: impl Into<String>,
        actions: Vec<TransactionAction<T>>,
    ) {
    ...
    pub(in crate::gui_app) fn can_undo(&self) -> bool {
        !self.undo.is_empty()
    }

    pub(in crate::gui_app) fn can_redo(&self) -> bool {
        !self.redo.is_empty()
    }

    pub(in crate::gui_app) fn is_transaction_open(&self) -> bool {
        self.active.is_some()
    }

    pub(in crate::gui_app) fn list_items(&self) -> Vec<TransactionListItem> {
...
impl GuiAppState {
    ...
    pub(in crate::gui_app) fn register_transaction_action(
        &mut self,
        label: impl Into<String>,
        undo: impl Fn(&mut GuiAppState) -> TransactionResult + 'static,
        redo: impl Fn(&mut GuiAppState) -> TransactionResult + 'static,
    ) {
        if self.transaction_restoring {
            return;
        }
    ...
    pub(in crate::gui_app) fn undo_transaction(&mut self) {
    ...
    pub(in crate::gui_app) fn redo_transaction(&mut self) {
        let mut history = std::mem::take(&mut self.transaction_history);
        let was_restoring = self.transaction_restoring;
        self.transaction_restoring = true;
    ...
    pub(in crate::gui_app) fn toggle_transaction_list(&mut self) {
