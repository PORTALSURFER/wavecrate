...
fn focus_loaded_toolbar_button_is_topmost_hit_target_and_paints_hover_feedback() {
    ...
    let icon_rect = frame
        ...
        .find_map(|primitive| match primitive {
            PaintPrimitive::Svg(svg) if svg.widget_id == super::super::TOOLBAR_FOCUS_LOADED_ID => {
                Some(svg.rect)
            }
            _ => None,
        })
...
fn stop_toolbar_button_is_hit_target_and_paints_hover_while_playing() {
    ...
    let icon_rect = frame
        ...
        .iter()
        .find_map(|primitive| match primitive {
            PaintPrimitive::Svg(svg) if svg.widget_id == super::super::TOOLBAR_STOP_ID => {
                Some(svg.rect)
            }
            _ => None,
        })
    ...
    assert!(
        !frame.paint_plan.primitives.iter().any(|primitive| matches!(
            ...
            PaintPrimitive::FillRect(fill)
                if fill.widget_id == super::super::WAVEFORM_WIDGET_ID
                    && fill.color.r == 71
                    && fill.color.g == 220
                    && fill.color.b == 255
        )),
