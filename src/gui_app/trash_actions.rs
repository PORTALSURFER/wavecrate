...
use super::{
    GuiAppState, GuiMessage, WaveformState, context_menu::BrowserContextTargetKind,
...
impl GuiAppState {
    ...
    pub(super) fn finish_trash_folder_dialog(&mut self, result: ui::PlatformResult) {
        let started_at = Instant::now();
    ...
    pub(super) fn clear_trash_folder(&mut self) {
        let started_at = Instant::now();
    ...
    pub(super) fn move_context_target_to_trash(&mut self) {
        let started_at = Instant::now();
    ...
    pub(super) fn move_selected_folder_to_trash(&mut self, path: PathBuf, started_at: Instant) {
        self.move_folder_path_to_trash(path, "folder_browser.delete_selected", started_at);
    }

    pub(super) fn move_selected_files_to_trash(
        &mut self,
        paths: Vec<PathBuf>,
        started_at: Instant,
    ) {
