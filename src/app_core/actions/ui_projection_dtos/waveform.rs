//! Waveform projection DTOs for the Wavecrate UI projection.
...
/// Normalized waveform viewport state exposed to the UI projection.
pub type WaveformViewportModel = visualization::TimelineViewport;

/// Waveform cursor, playhead, and selection transport state exposed to the UI projection.
pub type WaveformTransportModel = visualization::TimelineTransportState;

/// Waveform edit selection and fade-preview state exposed to the UI projection.
pub type WaveformEditPreviewModel = visualization::TimelineEditPreview;

/// One detected Wavecrate waveform slice preview exposed to the UI projection.
...
/// One-shot waveform feedback event tokens exposed to the UI projection.
pub type WaveformFeedbackEventsModel = visualization::TimelineFeedbackEvents;

/// Waveform guide/repeat/label presentation state exposed to the UI projection.
pub type WaveformPresentationModel = visualization::TimelinePresentationState;

/// Retained waveform raster preview state exposed to the UI projection.
pub type WaveformImagePreviewModel = visualization::SignalRasterPreview;

/// Waveform display chrome state exposed to the UI projection.
pub type WaveformChromeStateModel = visualization::SignalChromeState;

/// Waveform tool availability state exposed to the UI projection.
...
/// Aggregated waveform timeline surface state exposed to the UI projection.
pub type WaveformSurfaceModel = visualization::TimelineSurfaceState<WaveformSlicePreviewModel>;

/// Aggregated waveform motion state exposed to the UI projection.
