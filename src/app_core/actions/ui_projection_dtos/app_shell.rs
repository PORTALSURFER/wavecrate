//! Top-level UI projection application projection DTOs.
...
/// Progress overlay state projected into the UI projection.
pub type ProgressOverlayModel = feedback::ProgressOverlay;

/// Drag/drop overlay content for ui-projection feedback.
...
/// Update-check status projected into the UI projection.
...
/// Modal confirmation prompt projected into the UI projection.
pub type ConfirmPromptModel = feedback::ConfirmPrompt<ConfirmPromptKind>;

/// Prompt types that can block interaction in the UI projection.
...
/// Snapshot of Wavecrate state required by the UI projection renderer.
...
pub struct AppModel {
    ...
    /// Structured footer status segments used by the UI projection footer.
...
impl Default for AppModel {
    fn default() -> Self {
        Self {
            title: String::from(crate::native_runtime::DEFAULT_NATIVE_WINDOW_TITLE),
