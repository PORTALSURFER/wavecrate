...
use super::{
    PendingModelPullPreparation, WavecrateUiBridge,
    metrics::{
    ...
    projection_cache::{DerivedProjectionState, UiProjectionCacheKey},
};
use crate::app_core::{
...
impl WavecrateUiBridge {
    ...
    pub(super) fn projection_key_snapshot(&mut self) -> UiProjectionCacheKey {
    ...
    pub(super) fn pull_model_arc_snapshot(&mut self) -> Arc<NativeAppModel> {
        ...
        if call <= 24 && use_local_pull_fast_path {
            debug!(call, "UI bridge: pull_model using local-only fast path");
        }
        ...
        if call <= 24 {
            debug!(
                ...
                "UI bridge: pull_model completed"
            );
        }
    ...
    pub(super) fn project_motion_model_snapshot(&mut self) -> Option<NativeMotionModel> {
        ...
        let prepare_start = profiling.then(Instant::now);
        if call <= 24 {
            debug!(call, "UI bridge: project_motion_model start");
        }
        self.flush_pending_input_actions();
        if requires_full_pull {
            if call <= 24 {
                debug!(
                    call,
                    "UI bridge: project_motion_model escalated to full model pull"
                );
            }
            return None;
        }
        ...
        if call <= 24 {
            debug!(call, "UI bridge: project_motion_model completed");
        }
        if profiling && call.is_multiple_of(BRIDGE_PROFILE_INTERVAL) {
            maybe_log_bridge_profile();
        }
