...
use super::{
    PendingWaveformActions, WavecrateUiBridge,
...
impl WavecrateUiBridge {
    ...
    fn finalize_waveform_action_flush(
        ...
        before_key: UiProjectionCacheKey,
        emitted_actions: u64,
    ...
    pub(super) fn reduce_browser_focus_action(&mut self, delta: i8) {
        ...
        if call <= 64 {
            debug!(call, delta, "UI bridge: apply MoveBrowserFocus");
        }
    ...
    pub(super) fn reduce_immediate_waveform_preview_action(
        ...
        let action_start = profiling.then(Instant::now);
        if call <= 64 {
            debug!(call, action = ?action, "UI bridge: apply waveform preview action");
        }
    ...
    pub(super) fn reduce_queued_waveform_action(&mut self, action: &NativeUiAction) -> bool {
        ...
        let action_start = profiling.then(Instant::now);
        if call <= 64 {
            debug!(call, action = ?action, "UI bridge: queue waveform action");
        }
        if profiling {
            let action_duration =
                action_start.map_or(Duration::ZERO, |start: Instant| start.elapsed());
