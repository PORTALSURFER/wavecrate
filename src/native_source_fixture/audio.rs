...
pub(super) fn write_deterministic_wav(path: &Path, spec: &AudioSpec) -> Result<(), String> {
    ...
    let bytes = deterministic_wav_bytes(spec)?;
    std::fs::write(path, bytes)
        .map_err(|error| format!("write deterministic WAV {}: {error}", path.display()))
}

pub(super) fn deterministic_wav_bytes(spec: &AudioSpec) -> Result<Vec<u8>, String> {
    ...
    let mut output = Cursor::new(Vec::new());
    {
        let mut writer = hound::WavWriter::new(&mut output, wav_spec)
            .map_err(|error| format!("create deterministic WAV bytes: {error}"))?;
        for frame in 0..spec.frames {
            for channel in 0..u32::from(spec.channels) {
                ...
                writer
                    ...
                    .map_err(|error| format!("write deterministic WAV bytes: {error}"))?;
            }
        }
        writer
            .finalize()
            .map_err(|error| format!("finalize deterministic WAV bytes: {error}"))?;
    }
