//! Compatibility facade for the extracted `wavecrate-analysis` workspace crate.
//!
//! The main application crate continues to expose `crate::analysis` while the
//! implementation and heavy dependencies now live in `wavecrate-analysis`.

pub use wavecrate_analysis::analysis::{ann_index, hdbscan, similarity, umap, vector};
pub use wavecrate_analysis::{
...
pub(crate) mod audio {
    pub(crate) use wavecrate_analysis::{
...
pub(crate) mod version {
    pub(crate) use wavecrate_analysis::{analysis_version, analysis_version_for_sample_rate};
}
