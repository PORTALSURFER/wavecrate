...
/// Edit fade-in mute-handle updates should preserve length/curve and update only mute span.
#[test]
fn set_waveform_edit_fade_in_mute_start_milli_updates_edit_fade_in_mute() {
    let (mut controller, _source) = test_support::dummy_controller();
...
/// Edit fade-out mute-handle updates should preserve length/curve and update only mute span.
#[test]
fn set_waveform_edit_fade_out_mute_end_milli_updates_edit_fade_out_mute() {
    let (mut controller, _source) = test_support::dummy_controller();
    ...
    assert!((fade_out.mute - 0.25).abs() < 0.001);
}
