...
impl AppController {
    ...
    pub(super) fn defer_loaded_sample_duration_metadata_write(
        ...
        self.runtime.browser.pending_loaded_duration_metadata = Some(
    ...
    pub(crate) fn has_pending_loaded_duration_metadata_write(&self) -> bool {
        self.runtime
            .browser
            .pending_loaded_duration_metadata
            .is_some()
    }
    ...
    pub(crate) fn flush_pending_loaded_duration_metadata_write(&mut self) {
        ...
        let Some(pending) = self.runtime.browser.pending_loaded_duration_metadata.take() else {
            return;
        };
        if self.source_has_pending_file_mutations(&pending.source_id) {
            self.runtime.browser.pending_loaded_duration_metadata = Some(pending);
            self.runtime
                .browser
                .pending_loaded_duration_metadata_not_before =
                Some(Instant::now() + LOADED_DURATION_METADATA_DEBOUNCE);
