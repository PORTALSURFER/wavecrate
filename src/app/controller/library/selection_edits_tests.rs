...
#[test]
fn selection_fades_outer_gain_scales_outer_extensions() {
    let mut samples = vec![1.0_f32; 16];
    let fade_in = FadeParams::with_curve_mute_and_outer_gain(0.25, 0.0, 0.25, 0.5);
    let fade_out = FadeParams::with_curve_mute_and_outer_gain(0.25, 0.0, 0.25, 0.25);
    ...
    assert!(samples[3].abs() < 1e-6);
    assert!(samples[12].abs() < 1e-6);
    assert!((samples[13] - 0.25).abs() < 1e-6);
}
