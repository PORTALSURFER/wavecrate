use super::super::model::{ColumnRenderModel, SplitColumnRenderModel};
...
impl WaveformRenderer {
    ...
    pub(in crate::waveform::render) fn paint_color_image_for_size_with_density(
        ...
        if model.columns.is_empty() || width == 0 || height == 0 {
            return image;
        }
        ...
        let stepped = Self::stepped_columns(
            &model.columns,
            Self::horizontal_step(width, model.frames_per_column),
        );
    ...
    pub(in crate::waveform::render) fn paint_split_color_image_with_density(
        ...
        let fill =
            WaveformRgba::from_rgba_unmultiplied(background.r(), background.g(), background.b(), 0);
        let mut image = WaveformImage::new(
            [model.width as usize, model.height as usize],
            vec![fill; (model.width as usize) * (model.height as usize)],
        );
