...
- [x] 25) Split playback transport/player lifecycle helpers in `src/app/controller/playback/player.rs`
  - ROI/Effort: Medium / M
...
- 2026-03-10: Refreshed the cleanup audit against the current `next` head and revalidated the 30-item strict ROI backlog after items 14 and 15 landed.
- 2026-03-10: Revalidated the remaining top hotspots after the latest waveform/native-shell changes; item 16 is now the first pending step and the evidence counts above match the current head.
