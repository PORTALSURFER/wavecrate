...
Last Updated: 2026-03-17T13:45:00+01:00
Updated By: Codex
...
2. Keep `AGENTS.md`, `docs/plans/active/todo.md`, `docs/plans/index.md`, and this file aligned around the completed audit state.
3. Keep `tmp/cleanup_plan.md` and `tmp/perf_plan.md` parked unless the user explicitly reopens those lanes.
4. Use `scripts/ci_quick.ps1` as the default Windows validation gate before any future push and `scripts/ci_local.ps1` when broader parity is needed, noting that local Application Control currently blocks `cargo-nextest.exe`.
