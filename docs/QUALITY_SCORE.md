...
| Code size discipline | 3 | File size budget is enforced, the in-scope allowlist matches the current oversized `src/` and `tests/` debt, and `tmp/cleanup_audit_hotspots.md` tracks the broader hotspot set. |
...
- Reduce live file size allowlist debt: prioritize the remaining browser, updater, and large test hubs, and delete entries as files fall below 400 LOC.
