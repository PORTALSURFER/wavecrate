...
Last updated (local): 2026-04-02T23:06:00+02:00
...
- Phase 2 is in progress. Item 1 completed in commit `222eda75`, item 2 completed in commit `ffb70a38`, item 3 completed in vendor commit `75b6d980` plus superproject commit `ad3a487a`, item 4 completed in commit `41cee5b5`, and the migration-boundary preflight remains green.
- `powershell -ExecutionPolicy Bypass -File scripts/check_file_size_budget.ps1 -All` is currently red only on `src/app/controller/tests/browser_core/marks.rs`.
...
1. Implement item 5 from `tmp/improvement_audit_plan.md`: split `src/app/controller/tests/browser_core/marks.rs`.
