...
Last updated (UTC): 2026-03-08T03:05:00Z
...
- Perf Phase 2 items 1-9 in `tmp/perf_plan.md` are complete.
- The active ordered backlog lives in `tmp/perf_plan.md`.

## Next tasks (ordered)

1. Execute `tmp/perf_plan.md` item 10:
   replace proxy segment p95 attribution with true per-segment measured timings.
2. Execute `tmp/perf_plan.md` item 11:
   borrow `ShellLayout` through hot native input handlers instead of cloning it repeatedly.
3. Run a full pass over remaining perf-plan validation and handoff docs after items 10-11 land.
