...
pub(super) fn run(options: &BenchOptions) -> Result<AnalysisBenchResult, String> {
    ...
    for _ in 0..options.analysis_samples {
        ...
        let vec = wavecrate_analysis::compute_feature_vector_v1_for_mono_samples(
            &samples,
            options.analysis_sample_rate,
        )?;
        if vec.len() != wavecrate_analysis::FEATURE_VECTOR_LEN_V1 {
            source_err(vec.len())?;
        }
        if options.analysis_full {
            let embedding = wavecrate_analysis::compute_similarity_embedding_for_mono_samples(
                &samples,
                options.analysis_sample_rate,
            )?;
            if embedding.len() != wavecrate_analysis::similarity::SIMILARITY_DIM {
