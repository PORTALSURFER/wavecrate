//! Developer utility to build a similarity-map layout from stored embeddings.

use rusqlite::Connection;
use std::path::PathBuf;
use wavecrate_analysis_admin::cli_support;
...
fn run() -> Result<(), String> {
    ...
    let mut conn = conn;
    let report = wavecrate_analysis::build_map_layout(
        &mut conn,
        &options.model_id,
    ...
    println!(
        "Built similarity map layout for {} samples (coverage {:.2}%)",
        report.total,
        report.coverage_ratio * 100.0
    );
    let report_path =
        wavecrate_analysis::default_layout_report_path(&db_path, &options.umap_version);
    wavecrate_analysis::write_layout_report(&report_path, &report)?;
    println!(
...
fn help_text() -> String {
    [
        "wavecrate-umap",
        "",
        "Build a similarity-map layout for stored embeddings.",
        "",
        "Usage:",
        "  wavecrate-umap --model-id <id> --umap-version <version> [--db <path>] [--seed <u64>]",
        "",
        "Options:",
        "  --db <path>          Path to library.db (defaults to app data location).",
        "  --model-id <id>      Embedding model id to read (required).",
        "  --umap-version <v>   Layout version tag to store (required, legacy name).",
