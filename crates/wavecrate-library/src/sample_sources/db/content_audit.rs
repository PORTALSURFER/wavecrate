...
impl SourceDatabase {
    ...
    pub fn content_audit_retry_candidates(
        ...
        let mut candidates = collect_content_audit_candidates(
            &mut statement,
    ...
    fn content_audit_forward_window(
        ...
        let mut statement = self.connection.prepare(&sql).map_err(map_sql_error)?;
        collect_content_audit_candidates(
            &mut statement,
            params![
                cursor,
                rotation_id,
...
fn collect_content_audit_candidates<P>(
...
mod tests {
    ...
    fn measure_vm_callbacks(
        database: &SourceDatabase,
        operation: impl FnOnce() -> Result<Vec<SourceManifestEntry>, SourceDbError>,
    ) -> usize {
    ...
    fn forward_planning_vm_callbacks(total_entries: usize) -> usize {
        ...
        measure_vm_callbacks(&database, || {
            ...
            assert_eq!(candidates[0].relative_path, Path::new("sample-000000.wav"));
            Ok(candidates)
        })
    }
    ...
    fn retry_planning_vm_callbacks(total_entries: usize) -> (usize, usize) {
            ...
            .expect("fixture transaction");
        {
            ...
            for index in 0..total_entries {
                ...
                insert_retry
                    .execute(params![
                        path,
                        if index + 1 == total_entries { 1 } else { 1_000 }
                    ])
        ...
        let sparse_due = measure_vm_callbacks(&database, || {
            ...
            assert_eq!(
                ...
                PathBuf::from(format!("sample-{:06}.wav", total_entries - 1))
            );
            Ok(candidates)
        });
    ...
    fn content_audit_planning_stays_constant_at_large_manifest_scale() {
        ...
        assert!(
            forward_100k <= 50 && sparse_retry_100k <= 50 && no_retry_100k <= 50,
            "one-entry planning exceeded the 5,000 SQLite VM-instruction ceiling: \
             forward={forward_100k}, sparse retry={sparse_retry_100k}, \
             no retry={no_retry_100k} callbacks"
        );
    }
}
