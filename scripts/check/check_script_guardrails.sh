...
run_docs_index_fixture() {
  ...
  local repo_dir="$fixture_dir/repo"
  local script_path="$repo_dir/scripts/check/check_docs_index.sh"
  mkdir -p "$repo_dir/docs/plans/active" "$repo_dir/scripts/check" "$repo_dir/tmp"
  cp "scripts/check/check_docs_index.sh" "$script_path"
  chmod +x "$script_path"
...
run_file_size_budget_fixture() {
  ...
  local repo_dir="$fixture_dir/repo"
  local script_path="$repo_dir/scripts/check/check_file_size_budget.sh"
  mkdir -p "$repo_dir/src" "$repo_dir/scripts/check" "$repo_dir/vendor"
  cp "scripts/check/check_file_size_budget.sh" "$script_path"
  chmod +x "$script_path"
...
run_taste_invariants_fixture() {
  ...
  local repo_dir="$fixture_dir/repo"
  local script_path="$repo_dir/scripts/check/check_rust_taste_invariants.sh"
  mkdir -p "$repo_dir/src" "$repo_dir/scripts/check"
  cp "scripts/check/check_rust_taste_invariants.sh" "$script_path"
  chmod +x "$script_path"
...
run_run_contract_smoke_fixture() {
  ...
  local fixture_script="$ROOT_DIR/scripts/check/check_run_contract_smoke.sh"
...
run_memory_log_fixture() {
  ...
  local repo_dir="$fixture_dir/repo"
  local script_path="$repo_dir/scripts/check/check_memory_log.sh"
  mkdir -p "$repo_dir/scripts/check"
  cp "scripts/check/check_memory_log.sh" "$script_path"
  chmod +x "$script_path"
...
run_expect_exit_code \
  "bash -n scripts/agent/run_agent_ci_checks.sh" \
  0 \
  "$ROOT_DIR" \
  bash \
  -n \
...
run_expect_exit_code \
  "bash -n scripts/agent/run_agent_preflight.sh" \
  0 \
  "$ROOT_DIR" \
  bash \
  -n \
  scripts/agent/run_agent_preflight.sh
...
run_expect_exit_code \
  "bash -n scripts/agent/install_agent_preflight_hooks.sh" \
  0 \
  "$ROOT_DIR" \
  bash \
  -n \
  scripts/agent/install_agent_preflight_hooks.sh

run_expect_exit_code \
  "bash -n scripts/agent/refresh_memory_md.sh" \
  0 \
  "$ROOT_DIR" \
  bash \
  -n \
  scripts/agent/refresh_memory_md.sh
...
run_expect_exit_code \
  "bash -n scripts/check/check_file_size_budget.sh" \
  0 \
  "$ROOT_DIR" \
  bash \
  -n \
  scripts/check/check_file_size_budget.sh

run_expect_exit_code \
  "bash -n scripts/check/check_docs_index.sh" \
  0 \
  "$ROOT_DIR" \
  bash \
  -n \
  scripts/check/check_docs_index.sh
...
run_expect_exit_code \
  "bash -n scripts/check/check_rust_taste_invariants.sh" \
  0 \
  "$ROOT_DIR" \
  bash \
  -n \
  scripts/check/check_rust_taste_invariants.sh

run_expect_exit_code \
  "bash -n scripts/check/check_run_contract_smoke.sh" \
  0 \
  "$ROOT_DIR" \
  bash \
  -n \
  scripts/check/check_run_contract_smoke.sh

run_expect_exit_code \
  "bash -n scripts/check/check_rust_private_docs.sh" \
  0 \
  "$ROOT_DIR" \
  bash \
  -n \
  scripts/check/check_rust_private_docs.sh

run_expect_exit_code \
  "bash -n scripts/check/check_rust_dead_deps_advisory.sh" \
  0 \
  "$ROOT_DIR" \
  bash \
  -n \
  scripts/check/check_rust_dead_deps_advisory.sh

run_expect_exit_code \
  "bash -n scripts/perf/run_perf_guard.sh" \
  0 \
  "$ROOT_DIR" \
  bash \
  -n \
...
run_expect_exit_code \
  "bash -n scripts/perf/calibrate_startup_thresholds.sh" \
  0 \
  "$ROOT_DIR" \
  bash \
  -n \
  scripts/perf/calibrate_startup_thresholds.sh

run_expect_exit_code \
  "bash -n scripts/perf/run_perf_wheel_stability.sh" \
  0 \
  "$ROOT_DIR" \
  bash \
  -n \
  scripts/perf/run_perf_wheel_stability.sh
