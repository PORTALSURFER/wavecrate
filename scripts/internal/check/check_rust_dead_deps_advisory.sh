...
usage() {
  cat <<'USAGE'
Usage: scripts/internal/check/check_rust_dead_deps_advisory.sh [options]
...
Environment:
  WAVECRATE_DEAD_SWEEP_STRICT=1|true|yes|on
  WAVECRATE_DEAD_SWEEP_RUN_UDEPS=1|true|yes|on
  WAVECRATE_DEAD_SWEEP_INSTALL_MISSING=1|true|yes|on
  WAVECRATE_DEAD_SWEEP_UDEPS_TOOLCHAIN=<toolchain>
  WAVECRATE_DEAD_SWEEP_REPORT_PATH=<path>
USAGE
}
...
STRICT_MODE="$(to_bool "${WAVECRATE_DEAD_SWEEP_STRICT:-0}")"
RUN_UDEPS="$(to_bool "${WAVECRATE_DEAD_SWEEP_RUN_UDEPS:-0}")"
INSTALL_MISSING="$(to_bool "${WAVECRATE_DEAD_SWEEP_INSTALL_MISSING:-0}")"
UDEPS_TOOLCHAIN="${WAVECRATE_DEAD_SWEEP_UDEPS_TOOLCHAIN:-nightly}"
REPORT_PATH="${WAVECRATE_DEAD_SWEEP_REPORT_PATH:-}"
