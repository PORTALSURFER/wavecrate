...
usage() {
  cat <<'USAGE'
Usage: scripts/internal/agent/run_agent_ci_checks.sh [--refresh-memory] [--updater <name>] [--required-updater <name>] [--memory-max-age-hours <hours>]
...
if (( REFRESH_MEMORY == 1 )); then
  ./scripts/internal/agent/refresh_memory_md.sh "$UPDATER"
fi
...
run_check "memory log must be fresh (agent mode)" \
  ...
  "$REQUIRED_UPDATER" \
  ./scripts/internal/check/check_memory_log.sh
