...
STATE_ROOT="${WAVECRATE_PERF_WHEEL_STABILITY_ROOT:-$ROOT_DIR/target/perf/wheel_stability}"
WINDOWS="${WAVECRATE_PERF_WHEEL_STABILITY_WINDOWS:-7}"
RUNS_PER_WINDOW="${WAVECRATE_PERF_WHEEL_STABILITY_RUNS_PER_WINDOW:-3}"
REQUIRED_WINDOWS="${WAVECRATE_PERF_WHEEL_STABILITY_REQUIRED_WINDOWS:-7}"
TARGET_P95_US="${WAVECRATE_PERF_WHEEL_STABILITY_TARGET_P95_US:-16000}"
MAX_P95_SPREAD_US="${WAVECRATE_PERF_WHEEL_STABILITY_MAX_P95_SPREAD_US:-7000}"
MAX_STDDEV_US="${WAVECRATE_PERF_WHEEL_STABILITY_MAX_STDDEV_US:-6000}"
MAX_OUTLIER_RATIO="${WAVECRATE_PERF_WHEEL_STABILITY_MAX_OUTLIER_RATIO:-0.35}"
ENFORCE_READY="${WAVECRATE_PERF_WHEEL_STABILITY_ENFORCE_READY:-0}"
SUMMARY_PATH="${WAVECRATE_PERF_WHEEL_STABILITY_SUMMARY_OUT:-$STATE_ROOT/wheel_stability_summary.json}"
...
require_int "WAVECRATE_PERF_WHEEL_STABILITY_WINDOWS" "$WINDOWS"
require_int "WAVECRATE_PERF_WHEEL_STABILITY_RUNS_PER_WINDOW" "$RUNS_PER_WINDOW"
require_int "WAVECRATE_PERF_WHEEL_STABILITY_REQUIRED_WINDOWS" "$REQUIRED_WINDOWS"
require_int "WAVECRATE_PERF_WHEEL_STABILITY_TARGET_P95_US" "$TARGET_P95_US"
require_int "WAVECRATE_PERF_WHEEL_STABILITY_MAX_P95_SPREAD_US" "$MAX_P95_SPREAD_US"
require_int "WAVECRATE_PERF_WHEEL_STABILITY_MAX_STDDEV_US" "$MAX_STDDEV_US"
...
if [[ "$MODE" == "collect" || "$MODE" == "collect-and-evaluate" ]]; then
  if [ "$WINDOWS" -lt 1 ]; then
    echo "[wheel_stability] ERROR: WAVECRATE_PERF_WHEEL_STABILITY_WINDOWS must be >= 1" >&2
    exit 2
  fi
  if [ "$RUNS_PER_WINDOW" -lt 1 ]; then
    echo "[wheel_stability] ERROR: WAVECRATE_PERF_WHEEL_STABILITY_RUNS_PER_WINDOW must be >= 1" >&2
    exit 2
  fi
  ...
  for window in $(seq 1 "$WINDOWS"); do
    ...
    WAVECRATE_PERF_GUARD_OUT="$out_path" \
    WAVECRATE_PERF_GUARD_RUNS="$RUNS_PER_WINDOW" \
