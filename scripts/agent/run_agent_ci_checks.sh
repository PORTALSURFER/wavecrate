...
usage() {
  cat <<'USAGE'
Usage: scripts/agent/run_agent_ci_checks.sh [--refresh-memory] [--updater <name>] [--required-updater <name>] [--memory-max-age-hours <hours>]
...
if (( REFRESH_MEMORY == 1 )); then
  ./scripts/agent/refresh_memory_md.sh "$UPDATER"
fi
...
run_check "memory log must be fresh (agent mode)" \
  ...
  "$REQUIRED_UPDATER" \
  ./scripts/check/check_memory_log.sh
run_check "development branch policy" "" "" ./scripts/check/check_next_branch.sh
run_check "migration boundary guardrails" "" "" ./scripts/check/check_migration_boundary.sh
run_check "script guardrails" "" "" ./scripts/check/check_script_guardrails.sh
run_check "workflow toolchain pinning" "" "" ./scripts/check/check_workflow_toolchain_pinning.sh
run_check "manual docs scope guard" "" "" ./scripts/check/check_manual_docs_scope.sh
run_check "legacy app coupling guardrail" "" "" ./scripts/check/check_legacy_app_coupling.sh
run_check "rust todo/todo guardrail (non-test only)" "" "" ./scripts/check/check_rust_no_todos.sh
run_check "rust dead dependency/unused code sweep (advisory)" "" "" \
  ./scripts/check/check_rust_dead_deps_advisory.sh --advisory
run_check "rust public docs guardrail" "" "" ./scripts/check/check_rust_public_docs.sh
run_check "rust private docs guardrail" "" "" ./scripts/check/check_rust_private_docs.sh
run_check "app_core dependency boundary" "" "" ./scripts/check/check_app_core_dependency_boundary.sh
run_check "knowledge lint" "" "" ./scripts/check/knowledge_lint.sh
