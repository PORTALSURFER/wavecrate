...
.DESCRIPTION
PowerShell equivalent of `scripts/agent/run_agent_preflight.sh`.
...
if ($Help) {
  Write-Host "Usage: scripts/agent/run_agent_preflight.ps1 [-RefreshMemory] [-NoRefresh] [-Updater Codex] [-MemoryMaxAgeHours 1]"
  Write-Host ""
...
$checksScript = Join-Path $rootDir "scripts/agent/run_agent_ci_checks.ps1"
