...
try {
  if (-not (Get-Command bash -ErrorAction SilentlyContinue)) {
    throw "[wheel_stability] ERROR: bash is required for scripts/perf/run_perf_wheel_stability.sh"
  }
  & bash (Join-Path $rootDir "scripts/perf/run_perf_wheel_stability.sh") $Mode
} finally {
  Pop-Location
}
