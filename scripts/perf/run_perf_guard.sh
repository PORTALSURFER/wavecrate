...
STARTUP_LOCK_ENV_IN="${SEMPAL_PERF_GUARD_STARTUP_LOCK_ENV_IN:-$ROOT_DIR/scripts/perf/locks/startup_thresholds.env}"
...
if [[ -n "$FRAME_QUALITY_LOCK_ENV_OUT" ]]; then
  python3 scripts/perf/perf_frame_quality_lock_thresholds.py \
...
if (( startup_profile_enabled == 1 )); then
  ...
  startup_summary_cmd=(
    python3
    scripts/perf/perf_startup_summary.py
    --output
    "$startup_summary_out"
  ...
  if [[ -n "$STARTUP_LOCK_ENV_OUT" ]]; then
    ...
    python3 scripts/perf/perf_startup_lock_thresholds.py \
