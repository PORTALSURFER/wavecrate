...
#[test]
fn source_write_open_removes_invalid_relative_paths() {
    let dir = tempdir().unwrap();
    let db_file = dir.path().join(DB_FILE_NAME);
    {
        let conn = Connection::open(&db_file).unwrap();
...
#[test]
fn background_job_open_defers_invalid_relative_path_cleanup() {
    let dir = tempdir().unwrap();
    let db_file = dir.path().join(DB_FILE_NAME);
    {
        let conn = Connection::open(&db_file).unwrap();
    ...
    let fast = SourceDatabase::open_for_background_job(dir.path()).unwrap();
    ...
    let full = SourceDatabase::open_for_source_write(dir.path()).unwrap();
