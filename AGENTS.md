...
- Source of truth: `tmp/improvement_audit_plan.md` for the refreshed ROI-ranked backlog built from the current tree; `docs/gui_test_platform.md`, `tmp/cleanup_plan.md`, and `tmp/perf_plan.md` remain relevant background plans
- Current status: a fresh Phase 1 improvement audit was rebuilt on `2026-03-15` against `20f83666`; the backlog is waiting for explicit implementation approval.

## Immediate Next Actions
1. Wait for explicit user confirmation before implementing any item from `tmp/improvement_audit_plan.md`.
2. Keep `tmp/improvement_audit_plan.md` as the current source of truth for the refreshed improvement-audit backlog.
...
- `tmp/improvement_audit_plan.md`: refreshed evidence-driven ROI-ranked improvement backlog for the current tree; Phase 1 was rebuilt on `2026-03-15` and is awaiting explicit implementation approval
