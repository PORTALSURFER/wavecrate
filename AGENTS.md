...
- Current status: Phase 2 is active on `2026-04-03`. Items 1-3 are complete in commits `9fe71ec9` (`vendor/radiant`), `58e5fe24` (`vendor/radiant`), and `2bb31ea2` (`sempal`), the perf harness is green, and the ranked items continue in strict order from item 4.

## Immediate Next Actions
1. Treat `tmp/perf_plan.md` as the active runtime-performance source of truth for this lane while Phase 2 runs.
2. Continue with item 4 in `tmp/perf_plan.md`: remove path-clone-heavy whole-list work from the browser filter and mark lanes.
...
- `tmp/perf_plan.md`: active ROI-ranked runtime performance backlog for the live tree; items 1-3 are complete and item 4 is next
