...
- Use `main` as the direct working branch for `C:\dev\wavecrate`. Agent changes should be made, committed, and pushed directly on local `main`; do not create feature branches for ordinary Wavecrate work.
- Keep local `main` tracking `origin/main`; the repo hook installer and `scripts/check.* integration-branch` branch guard enforce that agent work is actually happening on `main`.
- `C:\dev\wavecrate\vendor\radiant` also uses local `main` tracking `origin/main`; update the submodule pointer from Wavecrate `main`.
...
- Intermediate commits and pushes to `main` are allowed without running the
  validation lanes. Use them to preserve progress; clearly report when pushed
  changes have not yet passed the final gate.
- Normal commits and pushes to `main` do not require the agent CI lane. Use
  focused checks or smoke checks when useful for the active change, and clearly
  report any checks that were skipped.
- Before preparing a versioned release on `main`, run formatting if code changed and run the
...
- After code changes: commit and push directly to `main` as useful for
  collaboration. Intentional versioned release commits still require the
