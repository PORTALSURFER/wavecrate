...
- Current status: Phase 2 is in progress on `2026-03-15`; items 1-2 are complete and the next planned task is the `browser_search.rs` decomposition.
...
2. Next up: item 3, split `src/app/controller/library/wavs/browser_search.rs` into focused modules for scoring/cache mechanics, async policy, and UI-trigger handlers.
...
- `tmp/improvement_audit_plan.md`: refreshed evidence-driven ROI-ranked improvement backlog and execution record; items 1-2 are complete as of `2026-03-15`
