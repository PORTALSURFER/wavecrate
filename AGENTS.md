...
- `tmp/bug_audit_plan.md`: 2026-04-18 clear-bug audit snapshot with Linear issue links for the Sempal project
...
- Keep both repos on local `next` tracking `origin/next`; the repo hook installer and `scripts/check/check_next_branch.*` are the enforcement path.
