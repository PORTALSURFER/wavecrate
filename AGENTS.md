...
- Current status: Phase 2 is active on `2026-04-03`. Items 1-2 are complete in `vendor/radiant` commits `9fe71ec9` and `58e5fe24`, the perf harness is green, and the ranked items continue in strict order from item 3.

## Immediate Next Actions
1. Treat `tmp/perf_plan.md` as the active runtime-performance source of truth for this lane while Phase 2 runs.
2. Continue with item 3 in `tmp/perf_plan.md`: stop cloning the entire retained native app model on every bridge projection miss.
...
- `tmp/perf_plan.md`: active ROI-ranked runtime performance backlog for the live tree; items 1-2 are complete and item 3 is next
