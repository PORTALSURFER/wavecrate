...
- Source of truth: `tmp/improvement_audit_plan.md` for the 2026-04-02 repo-wide ROI-ranked backlog and execution record; `docs/TEST.md` and `docs/README.md` still define the validation workflow; `docs/gui_test_platform.md` and `tmp/cleanup_audit_hotspots.md` are the key supporting references
- Current status: Phase 2 started on `2026-04-02`. Items 1 and 2 are implemented locally and validated; the remaining backlog is still executed strictly in plan order, and `tmp/cleanup_audit_hotspots.md` remains the supporting hotspot snapshot behind the ranked plan.

## Immediate Next Actions
1. Treat `tmp/improvement_audit_plan.md` as the active Phase 2 execution record for this lane.
2. Continue the backlog strictly in ranked order after item 2, unless a later item is blocked or needs clarification.
...
- `tmp/improvement_audit_plan.md`: active evidence-driven ROI-ranked repo-wide execution record for the live tree; items 1 and 2 are complete and the remaining items stay in ranked order
